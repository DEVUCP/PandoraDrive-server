    ...
    .flatMap { files =>
      ...
      val responseJson = Json.obj(
        ...
        "The number of photos/videos you uploaded this day" -> Json.fromInt(
          files.count(f =>
            f.created_at.exists(_.startsWith(currentDate("day"))) && (isPhoto(
              f.mime_type
            ) || isVideo(f.mime_type))
          )
        ),
        "The number of photos/videos you uploaded this week" -> Json.fromInt(
          files.count(f =>
            f.created_at.exists(_.startsWith(currentDate("week"))) && (isPhoto(
              f.mime_type
            ) || isVideo(f.mime_type))
          )
        ),
        "The number of photos/videos you uploaded this month" -> Json.fromInt(
          files.count(f =>
            f.created_at.exists(_.startsWith(currentDate("month"))) && (isPhoto(
              f.mime_type
            ) || isVideo(f.mime_type))
          )
        ),
        "The number of photos/videos you uploaded this year" -> Json.fromInt(
          files.count(f =>
            f.created_at.exists(_.startsWith(currentDate("year"))) && (isPhoto(
              f.mime_type
            ) || isVideo(f.mime_type))
          )
        ),
        "The length of your longest uploaded video" -> Json.fromInt(
          files
            .filter(f => isVideo(f.mime_type))
            .flatMap(_.duration_seconds)
            .maxOption
            .getOrElse(0)
        ),
